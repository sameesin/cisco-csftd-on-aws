---
title: "Firewall Configuration - Deployment"
weight: 2
---
...
This will give you all the private IPs of ELB, enter them when prompted. 

## Code Overview:
### <ins>Tapping in FMC </ins>
...
### <ins> Creating Access Control Policy </ins>
```python
...
### <ins> Adding Hosts & Network Objects </ins>
...
#APP1 Network Object
app1 = fmcapi.Networks(fmc=fmc, name="app1", value=app1_cidr)
app1.post()
        
#APP2 Network Object
app2 = fmcapi.Networks(fmc=fmc, name="app2", value=app2_cidr)
app2.post()

#APP LB IP Address 1
app_lb1 = fmcapi.Hosts(fmc=fmc, name="app-lb1", value=app1_lb_ip)
app_lb1.post()

#APP LB IP Address 2
app_lb2 = fmcapi.Hosts(fmc=fmc, name="app-lb2", value=app2_lb_ip)
app_lb2.post()
```

* Priavte IPs that were obtained from Step 1 will be passed here in elb_1 & elb_2.
* Choose the CIDR which is used by your web servers.
* Add the IP addresses of the loadbalancer attached to the web servers.

### <ins>Creating ACP rules</ins>
...
The NAT Rule above is for only one web server. 2nd NAT Rule will have the ```original_source``` as "elb2.name" & ```translated_destination``` as "app_lb2.name" 


### <ins> Adding FTD as a device </ins>
...
### <ins> Adding static routes on FTD for traffic to Webserver </ins>
...
### <ins> Associate NAT policy with FTD. </ins>
...
>Note: FMC Script can take upto 10 min to properly execute based on the wait time added in the code which can be modified.
